...
def run():
    ...
    log_model_path = os.path.join(model_uri, os.environ.get("MODEL_ARTIFACT_PATH", "artifacts"))
...
def invocations():
    ...
    try:
        if str(payload.get("stream", "false")).lower() == "true":
            ...
            return flask.Response(stream_response(), status=200, mimetype="text/event-stream;charset=UTF-8")
